...
export const UserProvider = ({ children }) => {
  ...
  const { logIn, signUp, logOut, client: supabase } = useAuth()
  ...
  //   const { id, email } = userMetadata(async function () {
  //     try {
  ...
  //         ? await supabase.from('users').match({ id }).single()
  //         : await supabase
  //             .from('users')
  ...
  const signOut = async () => {
    await supabase.from('users').match({ id: user }).single()
    setUser(null)
    logOut()
